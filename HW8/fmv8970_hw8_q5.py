class BinarySearchTreeMap:
        ...
        if(node_to_delete is self.root):
            ...
            if(num_children == 0):
                ...
                if(node_to_delete is parent.left):
                    parent.left = None
                    parent.left_nodes = 0 #nothing beyond because no kids
                else:
                    parent.right = None
                    parent.right_nodes = 0
    ...
    def get_ith_smallest(self, i):  # find key by keep track of the current rank
        if i > self.n or i < 1:
            raise IndexError("Um thats not a valid index... thats ugly of you")
        cursor = self.root
        rank_cursor = self.root.left_nodes + 1
        while rank_cursor != i:
            if rank_cursor > i:
                cursor = cursor.left
                rank_cursor -= cursor.right_nodes + 1
            else:
                cursor = cursor.right
                rank_cursor += cursor.left_nodes + 1
        return cursor.item.key

#bst = BinarySearchTreeMap
#bst.get_ith_smallest(bst,8)